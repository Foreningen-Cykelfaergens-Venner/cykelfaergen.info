<?
    if(isset($_COOKIE["region"]) && $_COOKIE["region"] == "dk-DK"){
        $title = "Referater | Foreningen Cykelfaergen's Venner";
...
<?
    ...
    <main class="content mt">
        <header class="underpage__header">
            <h1 class="board__headline --green">Referater</h1>
            <section class="intro">
        ...
        <section class="f__grid">
                <ul>
                <?
                    $sql = "SELECT * FROM referater";
                    $query = mysqli_query($db, $sql);

                    while($row = $query->fetch_assoc()){
                        $file = mb_convert_encoding($row["file"], 'UTF-8', 'HTML-ENTITIES');
		                $file = mb_convert_encoding($row["file"], "HTML-ENTITIES", "UTF-8");
