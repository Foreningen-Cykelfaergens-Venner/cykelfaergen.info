<?php
    ...
    }else if(isset($_COOKIE["region"]) && $_COOKIE["region"] == "da-DK" && $_SERVER["REQUEST_URI"] !== "/attraktioner"){
        header("Location: /attraktioner");
    }

    $mainHost = "www.".preg_replace("/^(.*?)\.(.*)$/","$2", $_SERVER["HTTP_HOST"]);
    $title = "Attractions around Flensburger fjord";
    include($_SERVER["DOCUMENT_ROOT"] . "/components/header.php");
?>
    <main class="container content">
        <h1>Attractions around Flensburger fjord</h1>
        <p>Find her en liste over attraktioner, hoteller og lignde, rundt om flensborg fjord, som du kan kombinere med en sejlads på cykelfærgen.</p>
